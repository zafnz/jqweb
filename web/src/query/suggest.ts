...
/* How to count what a query returns, which the output alone does not say:
   ...
   while everything else hands back a stream and the answer is how long it is.
...
/* A query ending in a name still being typed, as splitPartial reads it. */
...
/* The keys that could finish a partial name, as completions gathers them. */
export interface Completions {
  exact: boolean;
  objects: number;
  keys: { key: string; n: number }[];
}
...
function nodeAt(doc: Node, segs: Segment[]): Node | null {
  let n: Node | null = doc;
  for (let i = 0; i < segs.length && n; i++) {
    const seg = segs[i];
    if (seg.index !== undefined) {
      n = n.t === 'a' ? n.v[seg.index] : null;
    } else if (n.t === 'o') {
      const at = n.k.lastIndexOf(seg.key);
      n = at < 0 ? null : n.v[at];
    } else {
      n = null;
    }
...
function suffix(segs: Segment[]): string {
...
function depths(n: number): number[] {
  const out: number[] = [];
  for (let d = 0; d < n && d < MAX_PIVOTS; d++) out.push(d);
...
export function suggest(doc: Node, segs: Segment[]): Candidate[] {
  ...
  let literal: string | null = stringify(value);
  ...
  const inArray = literal !== null &&
    segs.length > 0 && segs[segs.length - 1].index !== undefined;
  /* Segments from a pivot's member down to the value's own container. */
  const below = inArray ? segs.slice(0, -1) : segs;

  function add(q: string, why: string, shape: Shape, rank: number): Candidate | null {
    if (!q || seen[q]) return null;
    seen[q] = true;
    const c: Candidate = { q: q, why: why, shape: shape, rank: rank };
    out.push(c);
    return c;
  }
  ...
  depths(segs.length).forEach(function (d) {
    ...
    const holds = inArray && d < below.length;
...
export function splitPartial(raw: string): PartialName | null {
  const m = /\.([A-Za-z_][A-Za-z0-9_]*)?$/.exec(raw);
  if (!m) return null;
  const lead = raw.slice(0, m.index);
  if (lead.charAt(lead.length - 1) === '.') return null;
  if (!lead && m[1] === undefined) return null;
  let ctx = lead.replace(/\s+$/, '');
...
export function completions(out: Node[], partial: string): Completions {
